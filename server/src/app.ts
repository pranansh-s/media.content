...
export function createApp(options: AppOptions = {}) {
  ...
  app.delete('/api/campaigns/:id', asyncHandler(async (req, res) => {
  ...
  app.post('/api/brands/:id/campaigns', asyncHandler(async (req, res) => {
          ...
          limit(async () => {
            ...
            try {
              ...
              if (asset.kind === 'text') {
                ...
                void ingestRevision(store, brand.id, asset.channel, body, embedder).catch(err => log.warn({ err }, 'rag ingest failed'));
              } else {
  ...
  app.post('/api/assets/:id/refine', asyncHandler(async (req, res) => {
  ...
  app.post('/api/assets/:id/regenerate', asyncHandler(async (req, res) => {
    const context = store.getAssetContext(String(req.params.id));
    if (!context) throw new NotFoundError('Asset not found');
  ...
  app.post('/api/assets/:id/revisions/:revisionId/restore', (req, res) => {
    ...
    const updated = source.kind === 'text' && 'body' in source
      ? store.appendRevision(context.asset.id, { ...revisionBase(null, 'manual'), body: source.body })
      : source.kind === 'image' && 'url' in source
        ? store.appendRevision(context.asset.id, { ...revisionBase(null, 'refined'), url: source.url, alt: source.alt })
        : null;
